...
## ledger_getChunks
根据高度范围批量查询快照块和快照块快照的账户块详情

- **Parameters**: 

    - `string`  查询的起始快照块高度（包含）
    - `string`  查询的结束快照块高度（包含）
...
- **Returns**: 
  - `SnapshotBlock`: 快照块详情
    ...
    - `nextSeedHash`: `Hash` 出块节点本轮生成的随机数的hash
    - `snapshotData` : `map[types.Address]HashHeight` 快照的账户块高度和hash
  - `AccountBlocks`: 快照块快照的账户块，参考[AccountBlockV1](https://vite.wiki/zh/api/rpcv1/common_models.html#AccountBlock)
