...
用户发起的交易类型不同，所需的配额也不同，预主网中，各种交易类型所需的配额见下表：
...
一个账户在发起一笔交易时的可用配额取决于UTPS、这个账户在过去74个快照块期间的配额使用情况和发起这笔交易时通过计算PoW获得的一次性配额，例如，一个账户通过抵押获得了1`UTPS`的配额，在过去74个快照块期间没有发起交易，并且计算了一个2`UTPS`难度的PoW，那么这个账户在发起一笔新交易时，最多可以使用77`UTPS`的配额。但是由于每个账户块有配额使用上限，上限为47.62`UTPS`，因此这笔交易实际只能使用47.62`UTPS`的配额。

为了计算方便，实际计算配额时，只计算$(\xi d \times \rho d)$或者$(\xi s \times \rho s)$的值，并将计算结果分段映射成相应的配额。映射表如下：
