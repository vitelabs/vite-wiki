...
如果用户希望一次性地发起一笔交易，可以选择计算一个${\rm PoW}$来一次性的获取配额；如果希望频繁发起交易，可以抵押一定数量的vite来获取持久的配额。
...
另外，对于转账交易中的备注，每个字符都需要收取额外的配额，每个字符收取68配额。

例如，如果用十六进制编码来表示备注，发起一笔备注为0x0001（共两个字符）的转账交易，需要的配额为：
...
|  $(\xi d \times \rho d +\xi s \times \rho s)$ | ${\it Q}$ | UTPS | 大约相当于在不算${\rm PoW}$的情况下抵押多少vite | 大约相当于在不抵押的情况下计算多少难度的${\rm PoW}$ |
|:------------:|:-----------:|:-----------:|:-----------:|:-----------:|
| 0.0 | 0 | 0 | 0 | 0 |
...
即，在不计算${\rm PoW}$的情况下，抵押10000vite可以获得最高1 ${\rm UTPS}$的交易频率，抵押20007可以获得最高2 ${\rm UTPS}$的交易频率。抵押134vite等待75个快照块（约1分钟25秒）也可以发起一笔不带备注的转账交易。
...
通过抵押的方式获取的配额数量跟抵押受益金额和发起交易前等待的快照块高度有关，抵押的vite如果没有取回，就可以持续获得配额。如果钱包中的当前配额展示为0，那么等待一段时间后展示的配额就会变多。
