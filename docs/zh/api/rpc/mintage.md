...
`Object`
  ...
  2. `height`: `uint64`  当前块高度
  3. `prevHash`: `Hash`  前一个账户块的哈希
  4. `tokenName`:`string`  代币名称，1-40个字符，包含大小写字母、空格和下划线，不能有连续空格，不能以空格开头或结尾
  5. `tokenSymbol`: `string` 代币简称，1-10个字符，包含大小写字母、空格和下划线，不能有连续空格，不能以空格开头或结尾
  6. `totalSupply`: `big.int` 总发行量，不能超过2**256-1
  7. `decimals`: `uint8` 小数位数，10**decimals不能超过totalSupply
  8. `isReIssuable`: `bool` 是否可增发，true-可增发 false-不可增发
  9. `maxSupply`: `uint256` 最大发行量，可增发代币必填，不能超过2**256-1，maxSupply>=totalSupply
  10. `ownerBurnOnly`: `bool` 是否仅支持所有者销毁，可增发代币必填，true-仅所有者可销毁 false-所有持币者可销毁
